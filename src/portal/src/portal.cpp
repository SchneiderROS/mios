...
std::shared_ptr<mirmi_utils::UDPStreamReceiver> Portal::open_udp_instream(const std::string &name, unsigned port, unsigned buffer_size, unsigned timeout_s, unsigned timeout_us, unsigned max_lost_packet, std::function<void(std::vector<double>&)> callback, bool multicast, const std::optional<std::string> &host){
    ...
    if(host.has_value()){
        m_instreams.insert(std::make_pair(name,std::make_shared<mirmi_utils::UDPStreamReceiver>(name,port,buffer_size,timeout_s,timeout_us,max_lost_packet,callback,multicast,host.value())));
    }
    else{
