...
def prefill_fast_pipe(iteration_n:int, kb_location:str, tags: list = ["10agents_25robots","ps_alpha_5"]):
    ...
    for results in data: 
        ...
        domain = Domain.from_dict(results["meta"]["doamin"])
        for i in range(1,len(results)-3+1):
            try:
                if results["n"+str(i)]["q_metric"]["success"]:
                    theta_normalised = domain.normalize(results["n"+str(i)]["theta"])
                    kb.push_trial(task_ident, theta_normalised, results["n"+str(i)]["q_metric"]["final_result"], 1000)
                else:
                    continue
            except KeyError:
                break
