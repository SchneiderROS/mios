...
void move_until_contact::initialize_task(){
    this->create_skill<move_to_contact>("move");
}
void move_until_contact::execute_task(){
    this->get_skill("move")->set_object("surface",this->surface);
    std::static_pointer_cast<ConfigSkill_move_to_contact>(this->get_skill("move")->get_config())->speed(0)=this->speed;
...
bool move_until_contact::read_parameters(const nlohmann::json& params){
    if(!msrm_utils::read_json_param(params,"speed",this->speed)){
        msrm_utils::print_error("Missing parameter: speed");
        return false;
    }
    if(!msrm_utils::read_json_param(params,"surface",this->surface)){
        msrm_utils::print_error("Missing parameter: surface");
        return false;
    }
return true;
}
