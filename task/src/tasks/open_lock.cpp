...
void open_lock::initialize_task(){
    this->create_subtask<insert_object>("insert_object");
    this->create_skill<turn>("turn");
    this->create_skill<extraction>("extract");
    this->create_skill<move_to_pose_cart>("turn_back");
...
void open_lock::execute_task(){
    ...
    if(!this->get_subtask("insert_object")->get_eval().success){
        msrm_utils::print_warning("I will not continue since the key insertion has failed.");
        return;
    }
    Object o;
    ...
    std::static_pointer_cast<ConfigSkill_turn>(this->get_skill("turn")->get_config())->angle(0)=this->angle;
...
bool open_lock::read_parameters(const nlohmann::json& params){
    if(!msrm_utils::read_json_param(params,"reverse",this->reverse)){
        this->reverse=true;
    }
    if(!msrm_utils::read_json_param(params,"release",this->release)){
        this->release=true;
    }
    if(!msrm_utils::read_json_param(params,"key",this->key)){
        msrm_utils::print_error("Missing parameter: key");
        return false;
    }
    if(!msrm_utils::read_json_param(params,"lock",this->lock)){
        msrm_utils::print_error("Missing parameter: lock");
        return false;
    }
    if(!msrm_utils::read_json_param(params,"angle",this->angle)){
        msrm_utils::print_error("Missing parameter: angle");
        return false;
    }
return true;
}
